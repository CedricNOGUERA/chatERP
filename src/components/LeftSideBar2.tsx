...
import { _getFriendInvite } from "../util/functionBis";

const LeftSideBar2 = ({ setRoomId }: any) => {
  ...
  const [frInvite, setFrInvite] = React.useState<any>([]);
...
// useEffect
  ...
  React.useEffect(() => {
    ...
    getGroup()
    subscribeFriendArea()
    _getFriendInvite(dataStore.id, setFrInvite)
  }, []);
  ...
  React.useEffect(() => {
    ...
    _getFriendInvite(selectedFriend?.id, setInvite)
  ...
  const invitationReceive = frInvite?.askers_list?.map((user: any) => allUser?.filter((dude: any, indx: any) => dude.id === user) )
...
_getUserData(setAllUser);
  ...
  const tri = friendAreas?.friends?.map((user: any) => allUser?.filter((dude: any, indx: any) => dude.id === user) )
        ...
        <div className="tab-pane active" id="chats" role="tabpanel">
          <div className="chat-room-list pt-3" data-simplebar>
                    ...
                    >
                      <Row className="w-100 ps-3">
                        <Col xs={2}>
                          <Avatar
                            avatar={user?.avatar}
                            badge={tri && tri[indx][0]?.badge_status}
                            statusUser={tri && tri[indx][0]?.status}
                          />
                        </Col>
            ...
            <div className="d-flex align-items-center mt-4 px-4 mb-2">
             ...
             <ul className="list-unstyled">
              {invitationReceive && invitationReceive?.map((user: any) =>
                    <li key={user[0]?.id} className="py-2">
                              ...
                              <Row className="px-0">
                                <Col xs={6}>
                                  <button
                                    ...
                                    className="btn btn-soft-success btn-sm shadow-none"
                                    onClick={() => {
                                      // acceptFriend(user[0]?.id_friend);
                                      handleShowDecision(user[0]?.id);
                                    }}
                                  >
